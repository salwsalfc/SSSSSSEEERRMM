...
async def ban_user(chat_id, i, rights):
    try:
        await co_od(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@co_od.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**▿∲ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@co_od.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@co_od.on(events.NewMessage(outgoing=False, pattern="/dev"))
async def _(event):
    user = await event.get_sender()
    if user.id == 5656828413:
        await event.reply("▿∲ اهـلا بـڪ مـطـوري أسامة \n▿∲ @WWWL5 ")
