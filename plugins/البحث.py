...
from . import co_od, song_download
...
@co_od.ar_cmd(pattern="بحث(320)?(?:\s|$)([\s\S]*)")
...
@co_od.ar_cmd(pattern="فيديو(?:\s|$)([\s\S]*)")
...
@co_od.ar_cmd(pattern="(ا(ل)?ا(س)?م)(?:\s|$)([\s\S]*)")
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await co_od(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
