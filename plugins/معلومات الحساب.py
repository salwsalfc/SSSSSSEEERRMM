...
async def stats(event):  # sourcery no-metrics
    ...
    response = f"♛ــــــــــــــــــــــــــــــــــــــــــــــــ♕ \n"
...
@co_od.on(admin_cmd(pattern="قائمه (جميع القنوات|قنوات اديرها|قنواتي)$"))
...
@co_od.on(admin_cmd(pattern="قائمه (جميع المجموعات|مجموعات اديرها|كروباتي)$"))
async def stats(event):  # sourcery no-metrics
    catcmd = event.pattern_match.group(1)
    catevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
