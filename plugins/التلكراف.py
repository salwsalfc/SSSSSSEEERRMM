...
from . import BOTLOG, BOTLOG_CHATID, co_od
...
@co_od.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    co_odevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await co_odevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await co_odevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
            os.remove(downloaded_file_name)
        else:
            end = datetime.now()
            ms = (end - start).seconds
            os.remove(downloaded_file_name)
            await co_odevent.edit(
        ...
        ms = (end - start).seconds
        co_od = f"https://telegra.ph/{response['path']}"
        await co_odevent.edit(
            f"**الرابط**:  [اضغط هنا]({co_od})\
