...
@co_od.ar_cmd(pattern="اكينوتر(?:\s|$)([\s\S]*)")
async def rozdo(e):
...
@co_od.tgbot.on(CallbackQuery(data=re.compile(b"aki_?(.*)")))
@check_owner
async def daj(e):
...
@co_od.tgbot.on(CallbackQuery(data=re.compile(b"aka_?(.*)")))
@check_owner
async def rooks(e):
