...
async def participant_check(channel, user_id):
    try:
        await co_od(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, co_od.uid):
            return await event.reply(
...
@co_od.ar_cmd(pattern="تعطيل الاجباري")
